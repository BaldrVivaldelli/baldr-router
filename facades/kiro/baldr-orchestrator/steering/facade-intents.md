...
This Kiro facade consumes the same versioned contract as every other Baldr client. It must not duplicate provider, workflow, Context7, telemetry, or verification logic.
...
| `run` | Create, continue, resume, or idempotently reuse the configured durable orchestration workflow for a task in the active workspace. | `@baldr /run <task>` | `/baldr-run <task>` | `run` |
