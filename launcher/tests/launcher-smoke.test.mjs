...
test('detect emits the shared runtime descriptor shape', () => {
  ...
  assert.equal(parsed.launcher.version, '0.16.1');
...
test('managed runtime manifest detects current wheel and upgrade mismatch', () => {
  ...
  const wheel = join(root, 'baldr_router-0.16.1.whl');
...
test('runtime upgrade pruning keeps current and newest prior version', () => {
  ...
  for (const [index, version] of ['0.13.0', '0.14.0', '0.15.0', '0.16.1'].entries()) {
    const dir = join(runtimeDir, version);
    fs.mkdirSync(dir, { recursive: true });
    const date = new Date(Date.now() - (4 - index) * 1000);
  ...
  assert.deepEqual(removed.sort(), ['0.13.0', '0.14.0']);
  assert.equal(fs.existsSync(join(runtimeDir, '0.15.0')), true);
  assert.equal(fs.existsSync(join(runtimeDir, '0.16.1')), true);
...
test('managed WSL runtime is created at its final path with rollback semantics', () => {
  ...
  const wheel = join(root, 'baldr_router-0.16.1-py3-none-any.whl');
  ...
  const capture = (command, args) => {
    ...
    if (command === 'wsl.exe') {
      ...
      if (script.startsWith('wslpath -u ')) {
        return { status: 0, stdout: '/mnt/c/baldr_router-0.16.1-py3-none-any.whl\n', stderr: '' };
      }
      if (script.includes('set -e;')) {
        installScript = script;
        return {
          status: 0,
          stdout: '/home/test/.local/share/baldr-router-vscode/0.16.1/venv/bin/baldr-router\n',
          stderr: '',
        };
      }
  ...
  assert.equal(target.ok, true);
  assert.equal(target.kind, 'wsl');
  assert.match(installScript, /root="\$HOME\/\.local\/share\/baldr-router-vscode\/0\.16\.1"/);
